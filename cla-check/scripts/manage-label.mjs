...
//
// Adds the signed label when every contributor has signed, and removes
// it otherwise. Idempotent: re-runs safely without duplicating the label
// or erroring if it's already in the desired state. Creates the label on
// first use so the action is self-contained in a fresh repo.
...
const LABEL_NAME = process.env.LABEL_NAME || 'cla-signed';
